...
class Blockchain():
    ...
    def addTransactionToPendingTransaction(self,transactionObj):
        # Check order_id existed or not
        exist = False
        for order in self.pendingTransactions:
            # if existed, insert transObj to order data
            if order['id']== transactionObj['id']:
                exist = True
                order['data'].append(transactionObj)
                break
        if exist == False:
            self.pendingTransactions.append({"id":transactionObj['id'],"data":[transactionObj]})
