...
class ApplyLoan(db.Model):
    ...
    order = db.relationship('Order',backref='apply_loan',uselist=False,lazy=True,cascade=None)
    pay_installment = db.relationship('PayInstallment',backref='apply_loan')
    def __repr__(self):
        return f'<ApplyLoan order_id={self.order.id} >'

class PayInstallment(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    created_at = db.Column(db.DateTime, server_default=db.func.now())
    updated_at = db.Column(db.DateTime, server_default=db.func.now(), server_onupdate=db.func.now())
...
def get_orders(by,value):
    ...
    for order in orders:
        if order['transactions'][0]['data'][-1][by] == value and order['transactions'][0]['id'] not in temp_id:
            action = order['transactions'][0]['data'][-1]['action']
            ...
            # if order['transactions'][0]['data'][-1]['applyloan_id'] != None:
            a = ApplyLoan.query.filter_by(id=order['transactions'][0]['data'][-1]['applyloan_id']).first()
...
def get_pay_installment(id):
    o = get_order(int(id))
    pay = o.apply_loan.pay_installment

    pay_schema = PayInstallmentSchema(many=True)
    return make_response(jsonify(pay_schema.dump(pay)))

# add payment installment untuk cicilan pinjaman
@app.route('/api/pay_installment/add_payment',methods=['POST'])
def add_pay_installment():
    file = request.files['pay_installment_file']
    if file.filename.endswith('jpeg') or file.filename.endswith('jpg') or file.filename.endswith('png') or file.filename.endswith('pdf') or file.filename.endswith('PNG'):
        ID = uuid.uuid4().hex
...
def get_order_endpoint(id):
    o = get_order(int(id))
    if o.action == 'create_order':
        action = 1
    elif o.action == 'sign_by_supplier':
        action = 2
    elif o.action == 'enterprise_upload_voucher':
        action = 3
    elif o.action == 'apply_loan':
        action = 4
