$(document).ready(function () {
    var price = jumlahBarang = discount = totalPrice = totalDisc =  totalAmountDiscount = 0
    let namaPemesan = noHpPemesan = idBarang = idJenisPembayaran = idEnterprise = ''

    function alertBootstrap(message, type) {
        let alert = $(`<div class="alert alert-${type} alert-dismissible" role="alert"><div>${message}</div><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>`)
    ...
    $('#id_enterprise').on('change', function () {
    ...
    $('#confirmationModal .modal-footer #agree').on('click', function (event) {
        ...
        let payload = {
            ...
            "totalDiskon":  totalAmountDiscount
        }
        addOrder(payload)
    });
    ...
    function getDetailOrder(id) {
        ...
        .done(function(d) {
            ...
            $('#do_jumlah_barang').text(formatNumer(data.jumlah_barang))
            $('#do_jenis_bayar').text(data.jenis_bayar)
            $('#do_total_discount').text(`Rp ${formatNumer(data.total_diskon)}`)
            $('#do_total_harga').text(`Rp ${formatNumer(data.total_harga)}`)
            $('#detailOrderModal').modal('show')
        })
    }
    ...
    function checkValue() {
        let disable = true;
        if (idEnterprise !== '' && namaPemesan !== '' && noHpPemesan !== '' && idBarang !== '' && idJenisPembayaran !== '' && jumlahBarang !== 0) {
            disable = false
        }
        if (disable) {
